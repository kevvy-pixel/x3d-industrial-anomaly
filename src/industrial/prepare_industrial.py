...
def main() -> None:
    ...
    for group in source_groups:
        group_name = group["name"]
        selected = [item for item in sources if item["source_group"] == group_name]
        rng.shuffle(selected)
        if len(selected) < folds:
            raise RuntimeError(f"{group_name} has {len(selected)} sources, fewer than {folds} folds")
        for index, item in enumerate(selected):
            fold_assignment[item["source_id"]] = index % folds
    ...
    audit = {"seed": int(cfg["seed"]), "folds": folds, "classes": classes, "folds_detail": {}}
    for test_fold in range(folds):
        val_fold = (test_fold + 1) % folds
        split_sources = {"train": [], "val": [], "test": []}
        for source in sources:
            assigned = fold_assignment[source["source_id"]]
            split = "test" if assigned == test_fold else "val" if assigned == val_fold else "train"
            split_sources[split].append(source)
        split_rows = {split: [row for source in values for row in samples_for(source)] for split, values in split_sources.items()}
        for split, rows in split_rows.items():
            rows.sort(key=lambda row: (int(row["label"]), row["source_id"], row["sample_id"]))
            write_csv(manifest_root / f"fold_{test_fold}" / f"{split}.csv", rows)
        source_sets = {split: {item["source_id"] for item in values} for split, values in split_sources.items()}
        overlaps = {
        ...
        if any(overlaps.values()):
            raise RuntimeError(f"source leakage in fold {test_fold}: {overlaps}")
        audit["folds_detail"][str(test_fold)] = {
            ...
            "val_sources": sorted(source_sets["val"]),
        }
    audit["all_sources"] = len(sources)
    audit["all_samples"] = sum(len(samples_for(source)) for source in sources)
    audit["fold_assignment"] = fold_assignment
